...
- 📄 **BabelDOC 排版对照 PDF**：一键生成原版排版+译文的对照 PDF（随包内置运行时，开箱即用；单并发排队 + 实时进度）；**上传时可选「BabelDOC 对照」模式**——跳过重排版直接生成对照 PDF（版式还原更稳，仅英→中）
...
| `v0.14.4` | 验收修复+体验 | 关闭页签误入未达标翻译中文档（门禁单点化）、上传可选「BabelDOC 对照」模式 | 🚧 代码完成待发布 |
